...
The load order in `.zshrc` is deliberate — for example `compinit` has to run after `zsh-completions` is loaded, and `aliases.zsh` has to precede `functions/` because the global `-h`/`--help` aliases are expanded while the function bodies are parsed. Each module documents its own constraints at the top; read those before reordering.

Drop a new file into `.config/zsh/functions/` to add functions — no re-stow and no loader change needed, since the directory is a single folded symlink and the files are picked up by a glob.
